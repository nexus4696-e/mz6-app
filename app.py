...
def notify_staff_via_line(token, target_id, staff_name, cast_name, pickup_time):
    ...
    try: requests.post(url, headers=headers, json={'to': target_id, 'messages': [{'type': 'text', 'text': msg}]}, timeout=5)
    except: pass
...
def notify_driver_route_via_line(token, target_id, driver_name, route_details, dep_time):
    ...
    for idx, rt in enumerate(route_details):
        msg += f"[{idx+1}] {rt['time']} {rt['name']}\n📍 {rt['addr']}\n\n"
    msg += "安全運転でお願いします！"
    try: requests.post(url, headers=headers, json={'to': target_id, 'messages': [{'type': 'text', 'text': msg}]}, timeout=5)
    except: pass
...
def recalc_route_for_driver(drv_name, trigger_line_notify=False):
    ...
    ordered_tasks, total_sec, full_path, first_leg_sec, _ = optimize_and_calc_route(GOOGLE_MAPS_API_KEY, store_addr_f, store_addr_f, valid_drv_tasks, is_return=False)
...
if st.session_state.page == "home":
    ...
    if st.button("スタッフ業務開始\n（配車・送迎設定）", use_container_width=True):
        st.session_state.page = "staff_login"; st.rerun()
    ...
    if st.button("キャスト専用ログイン\n（予定の申請）", use_container_width=True):
        st.session_state.page = "cast_login"; st.rerun()
    st.markdown("<br>", unsafe_allow_html=True)
...
# ==========================================
    ...
    db = get_db_data(); casts = db.get("casts", []); drvs = db.get("drivers", [])
    atts = db.get("attendance", []); sets = db.get("settings") or {}
    ...
    if not is_adm:
                    ...
                    if st.button("変更を反映", type="primary", use_container_width=True):
                        ...
                        if len(my_assigned) > n_cap:
                            ...
                            for r in my_assigned:
                                ...
                                addr = clean_address_for_map(c_inf.get("address", ""))
                                _, d_score = get_route_line_and_distance(addr)
                                with_dist.append({"row": r, "dist": d_score})
                            with_dist.sort(key=lambda x: x["dist"]) # 近い順に外す
                            ...
                            for oc in overflow_casts:
                                ...
                                for d in drvs:
                                    ...
                                    d_cap_check = int(d.get("capacity", 4))
                                    d_assigned = [r for r in atts if r["target_date"] == "当日" and r["status"] in ["出勤", "自走"] and r.get("driver_name") == d["name"]]
                                    if len(d_assigned) < d_cap_check: assigned_d = d["name"]; break
                            ...
                            if updates:
                                ...
                                time.sleep(0.5)
                                recalc_route_for_driver(staff_n, trigger_line_notify=True)
                                for u in updates:
                                    if u["driver_name"] != "未定": recalc_route_for_driver(u["driver_name"], trigger_line_notify=True)
        ...
        if my_early:
            ...
            elif first_leg_sec == 0: early_html += f"<div style='font-size:14px; font-weight:bold; color:white; background:#f44336; padding:8px; border-radius:5px; margin-bottom:10px; text-align:center;'>🚨 Google API通信エラー:<br>{api_err if api_err else '距離計算失敗'}</div>"
            else:
                earliest_m = 9999
                for rt in ord_early:
                    try:
            ...
            if early_path:
                ...
                early_html += f"<a href='https://www.google.com/maps/dir/?api=1&origin={org_enc}&destination={d_enc}&travelmode=driving&waypoints={wp_enc}' target='_blank' style='{NAV_BTN_STYLE} background:#ff9800; margin-bottom:10px;'>🗺️ 早便ナビ開始</a>"
        ...
        if my_tasks:
            ...
            list_html = f'<div style="background:#444; color:white; padding:10px; font-weight:bold; border-radius:5px 5px 0 0;">🚕 通常便ルート</div><div style="background:#ffffff; border:1px solid #ccc; border-top:none; padding:10px; border-radius:0 0 5px 5px; margin-bottom:20px; box-shadow: 0 2px 5px rgba(0,0,0,0.05);">'
            
            if is_return_time:
                list_html += f'<div style="background:#e3f2fd; border:2px solid #2196f3; padding:8px; border-radius:5px; margin-bottom:15px;"><div style="color:#1565c0; font-weight:bold; margin-bottom:5px;">🌙 帰り班 (自動編成)</div>'
                return_tasks = []
                ...
                elif first_leg_sec == 0: list_html += f"<div style='font-size:14px; font-weight:bold; color:white; background:#f44336; padding:8px; border-radius:5px; margin-bottom:10px; text-align:center;'>🚨 Google API通信エラー:<br>{api_err if api_err else '距離計算失敗'}</div>"
                else:
                    earliest_m = 9999
    ...
    # 👑 管理者フル機能
        ...
        if selected_tab == "① 配車リスト":
                        ...
                        if not all_today_casts:
                            ...
                            for uc in all_today_casts:
                                ...
                                for d_name, stat in drv_specs.items():
                                    ...
                                    score = 0
                                    if stat["line"] == c_line: score += 100
                                    elif stat["line"] is None:
                                        if driver_covers_line(stat["area"], c_line): score += 50
                                        else: score -= 50
                                    else:
                                ...
                                if best_d: assigned_d = best_d
                                    ...
                                    for d_name, stat in drv_specs.items():
                                        if len(stat["assigned_rows"]) < stat["capacity"]: assigned_d = d_name; break
                            ...
                            for d_name, stat in drv_specs.items():
                                ...
                                ordered_tasks, total_sec, full_path, first_leg_sec, api_err = optimize_and_calc_route(GOOGLE_MAPS_API_KEY, store_addr, store_addr, ai_tasks, is_return=False)
                                
                                if total_sec == 0:
                                    st.warning(f"⚠️ API通信エラー({api_err if api_err else '距離計算失敗'}): {d_name}班の計算に失敗しましたが、配車割り当ては強制実行しました。")
                                ...
                                for idx, item in enumerate(ordered_tasks):
                                    ...
                                    route_details.append({
                                        "name": item["c_name"], "time": current_calc_time, "addr": item["actual_pickup"],
                                        "cast_line_id": next((c.get("line_user_id", "") for c in casts if str(c["cast_id"]) == str(item["c_id"])), "")
                                    })
                                
                                driver_routes_info[d_name] = {
                                    "dep_time": dep_time_str, "route": route_details,
                                    "driver_line_id": next((d.get("line_user_id", "") for d in drvs if d["name"] == d_name), "")
                                }
            ...
            if unassigned:
                unassigned_html = '<div class="warning-box">⚠️ 定員・エリアオーバーで未割り当てのキャスト</div><div class="warning-content"><div style="font-size:12px; color:#666; margin-bottom:10px;">※下の「全キャスト検索」から手動で割り当てるか、稼働ドライバーを追加してください。</div>'
                for u in unassigned:
                    c_info = next((c for c in casts if str(c["cast_id"]) == str(u["cast_id"])), {})
                    latest_name = c_info.get("name", u["cast_name"])
                    unassigned_html += f"<div style='margin-bottom:5px;'><b>未定</b>　<span style='font-size:16px; font-weight:bold;'>{latest_name}</span> <br><span style='font-size:12px; color:#555;'>({u['status']})</span></div><hr style='margin:5px 0;'>"
                unassigned_html += '</div>'
            ...
            for d_name, t_rows in my_tasks.items():
                ...
                st.markdown(f'<div style="background:#444; color:white; padding:10px; font-weight:bold; border-radius:5px 5px 0 0;">🚕 コース{course_idx}：{d_name} (STAFF)</div><div style="background:#ffffff; border:1px solid #ccc; border-top:none; padding:10px; border-radius:0 0 5px 5px; box-shadow: 0 2px 5px rgba(0,0,0,0.05);">', unsafe_allow_html=True)
                ...
                if is_return_time:
                    ...
                    if return_full_path:
                        org_enc = urllib.parse.quote(store_addr); dest_enc = urllib.parse.quote(store_addr); wp_enc = urllib.parse.quote("|".join(return_full_path[:-1])) if len(return_full_path) > 1 else ""
                        st.markdown(f"<a href='https://www.google.com/maps/dir/?api=1&origin={org_enc}&destination={dest_enc}&travelmode=driving&waypoints={wp_enc}' target='_blank' style='{NAV_BTN_STYLE} background:#1565c0; margin-bottom:10px;'>🗺️ 帰りナビ開始 (現在地から)</a>", unsafe_allow_html=True)
                    for idx, rt in enumerate(ordered_returns):
                    ...
                    elif first_leg_sec == 0: st.markdown(f"<div style='font-size:14px; font-weight:bold; color:white; background:#f44336; padding:8px; border-radius:5px; margin-bottom:10px; text-align:center;'>🚨 Google API通信エラー:<br>{api_err if api_err else '距離計算失敗'}</div>", unsafe_allow_html=True)
                    else:
                        earliest_m = 9999
                        for t in ordered_tasks:
                            try:
                ...
                with st.expander(f"🔄 コース{course_idx}（{d_name}班）の担当・キャスト変更"):
                    ...
                    with col_drv2:
                        if st.button("入替", key=f"btn_course_{d_name}_{course_idx}", use_container_width=True):
                            if new_course_drv != "--" and new_course_drv != d_name:
                                updates = []
                                for r in atts:
                                    if r["target_date"] == "当日" and r["status"] in ["出勤", "自走"]:
                                        if r.get("driver_name") == d_name:
                                            updates.append({"id": r["id"], "driver_name": new_course_drv, "pickup_time": "未定", "status": r["status"]})
                                        elif r.get("driver_name") == new_course_drv:
                                            updates.append({"id": r["id"], "driver_name": d_name, "pickup_time": "未定", "status": r["status"]})
                                if updates:
                                    post_api({"action": "update_manual_dispatch", "updates": updates})
                                    time.sleep(0.5); clear_cache()
                                    recalc_route_for_driver(d_name, trigger_line_notify=True)
                                    recalc_route_for_driver(new_course_drv, trigger_line_notify=True)
                                    clear_cache(); st.rerun()
                    
                    st.markdown("<hr style='margin:10px 0;'>", unsafe_allow_html=True)
                    st.markdown("**◆ 個別キャストを別の車へ移動**")
                    for t in t_rows:
                        c_info = next((c for c in casts if str(c["cast_id"]) == str(t["cast_id"])), {})
                        latest_name = c_info.get("name", t['cast_name'])
                        col_ind1, col_ind2 = st.columns([3, 1])
                        with col_ind1: new_ind_drv = st.selectbox(f"{latest_name}", ["--", "未定"] + d_names, key=f"ind_drv_{t['id']}_{course_idx}")
                        with col_ind2:
                            st.markdown("<div style='margin-top:28px;'></div>", unsafe_allow_html=True)
                            if st.button("移動", key=f"btn_ind_{t['id']}_{course_idx}", use_container_width=True):
                                if new_ind_drv != "--" and new_ind_drv != d_name:
                                    post_api({"action": "update_manual_dispatch", "updates": [{"id": t["id"], "driver_name": new_ind_drv, "pickup_time": "未定", "status": t.get("status")}]})
                                    time.sleep(0.5); clear_cache()
                                    recalc_route_for_driver(d_name, trigger_line_notify=True)
                                    if new_ind_drv != "未定": recalc_route_for_driver(new_ind_drv, trigger_line_notify=True)
                                    clear_cache(); st.rerun()
