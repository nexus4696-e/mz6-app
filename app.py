...
def optimize_and_calc_route(api_key, store_addr, dest_addr, tasks_list, is_return=False):
    ...
    if full_path:
        ...
        # 🌟 departure_time="now" を付与し、AIにリアルタイムの渋滞予測と学習を強制
        params = {
            "origin": calc_origin,
                ...
                if n_s not in ["未定", "休み"]:
                    ...
                    if new_row:
                        ...
                        if n_d != "未定" and n_d != cur_drv:
                            bot_id = sets.get("line_bot_id", "")
                            l_token = sets.get("line_access_token", "")
                            notify_staff_via_line(l_token, bot_id, n_d, latest_name, n_t)
        ...
        if st.session_state.current_staff_tab == "① 配車リスト":
            ...
            if st.button("🚀 AI自動配車 (ゼロベース再編成)", type="primary", use_container_width=True):
                ...
                if valid_count == 0:
                    ...
                    # （※ダミーの自動割り当てデモ。本来のAPI実装が走る箇所）
                    st.success("最短ルート最適化完了！")
                    # 🌟 実際の自動配車ロジック後、スタッフへLINE通知を飛ばす
                    bot_id = sets.get("line_bot_id", "")
                    l_token = sets.get("line_access_token", "")
                    # notify_staff_via_line(l_token, bot_id, driver_name, cast_name, pickup_time)
                    clear_cache(); st.rerun()
