...
def optimize_and_calc_route(api_key, store_addr, dest_addr, tasks_list, is_return=False):
    ...
    if is_return:
        valid_tasks.sort(key=lambda x: x["dist_score"]) # 帰り便：店から近い順
    else:
        valid_tasks.sort(key=lambda x: x["dist_score"], reverse=True) # 迎え/早便：店から遠い順
...
def render_cast_edit_card(c_id, c_name, pref, target_row, prefix_key, d_names_list, t_slots, e_t_slots, loop_idx):
    ...
    latest_name = c_info_full.get("name", c_name)
                ...
                if res1.get("status") == "success":
                    ...
                    if new_status not in ["未定", "休み"]:
                        ...
                        if new_row:
                            ...
                            # 🌟 スタッフへのLINE通知発行
                            if new_drv != "未定" and new_drv != cur_drv:
                                stff_id = next((d.get("line_user_id", "") for d in db_temp.get("drivers", []) if d["name"] == new_drv), "")
                                notify_staff_via_line(sets.get("line_access_token", ""), stff_id, new_drv, latest_name, new_time)
                                
                            time.sleep(0.5)
                            clear_cache()
                    
                    # 🌟 検索窓の入力内容をリセットし、「更新しました」状態に切り替える
                    ...
                    time.sleep(0.5)
                    st.session_state.flash_msg = f"{latest_name} の情報を更新しました！"
                    st.rerun() 
                else:
...
# ==========================================
    ...
    latest_name = my_cast_info.get("name", c["キャスト名"]) if my_cast_info else c["キャスト名"]

    st.markdown(f'''
    <div style="text-align: center; font-weight: bold; font-size: 20px; margin-bottom: 5px;">
        店番 {c['店番']}　{latest_name} 様<br><span style="font-size: 14px; color: #555; font-weight: normal;">(担当: {c.get('担当', '未設定')})</span>
    </div>
    ''', unsafe_allow_html=True)
    ...
    with tab_today:
        ...
        with col_t2:
            ...
            if st.button("📤 送信", type="primary", use_container_width=True, key="btn_submit_today"):
                ...
                rec = {"cast_id": c["店番"], "cast_name": latest_name, "area": c["方面"], "status": today_s, "memo": encoded_memo, "target_date": "当日"}
    ...
    with tab_tmr:
        ...
        with col_tm2:
            ...
            if st.button("📤 送信", type="primary", use_container_width=True, key="btn_submit_tmr"):
                ...
                rec = {"cast_id": c["店番"], "cast_name": latest_name, "area": c["方面"], "status": tmr_s, "memo": encoded_memo_tmr, "target_date": "翌日"}
    ...
    if not is_admin:
        ...
        if my_early_tasks:
            ...
            if earliest_target_mins != 9999:
                ...
                if early_total_sec == 0:
                    travel_sec = len(ordered_early_tasks) * 15 * 60
                else:
                    travel_sec = early_total_sec
                    
                total_travel_mins = (travel_sec + padding_sec) // 60
        ...
        if not my_tasks_raw:
            ...
            if is_return_time:
                ...
                try:
                    ...
                    if total_sec == 0:
                        travel_sec = len(ordered_tasks) * 15 * 60
                    else:
                        travel_sec = total_sec
                        ...
                        if not all_today_casts:
                            ...
                            for d_name, stat in drv_specs.items():
                                ...
                                for idx, item in enumerate(ordered_assigned):
                                    ...
                                    c_info_line = next((c for c in casts if str(c["cast_id"]) == str(item["row"]["cast_id"])), {})
                                    l_name = c_info_line.get("name", item["row"]["cast_name"])
                                    notify_staff_via_line(settings.get("line_access_token", ""), stff_id, d_name, l_name, current_calc_time)
