...
# 🌟 【完全修正】無駄な行ったり来たりを無くす「一筆書き最適化」Google AIアルゴリズム
@st.cache_data(ttl=120)
def optimize_and_calc_route(api_key, store_addr, dest_addr, tasks_list, is_return=False):
    if not tasks_list: return [], 0, []

    valid_tasks = [t for t in tasks_list if clean_address_for_map(t.get("actual_pickup", ""))]
    ...
    if api_key and len(valid_tasks) > 1:
        # 🌟 Googleの力を借りて「一筆書きで一番無駄のない順番(optimize:true)」を計算してもらう
        wp_str = "optimize:true|" + "|".join([clean_address_for_map(t["actual_pickup"]) for t in valid_tasks])
        try:
            ...
            if res.get("status") == "OK":
                ...
                # 🌟 一筆書きルートの中で「遠い人から拾う」ルールになるように向きを調整する
...
# ==========================================
    ...
    if not is_admin:
        ...
        if my_early_tasks:
            ...
            if earliest_target_mins != 9999:
                ...
                # 🌟 API未設定・エラー時の完全フェイルセーフ（移動時間を1件あたり15分確保）
    ...
    # ========================================================
        ...
        # 🚨 タブ切り替え時の ValueError を完全に防ぐ保護ロジック
        tabs_list = ["① 配車リスト", "② キャスト送迎", "③ キャスト登録", "④ STAFF設定", "⚙️ 管理設定"]
