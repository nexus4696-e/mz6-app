...
# ==========================================
    ...
    db = get_db_data()
    casts = db.get("casts", []); drvs = db.get("drivers", []); atts = db.get("attendance", []); sets = db.get("settings") or {}
                    ...
                    if not GOOGLE_MAPS_API_KEY: st.error("🚨 API通信エラー: Google Maps APIキーが読み込めないため、AI自動配車は実行できません。")
                        ...
                        if not all_today_casts:
                            ...
                            for d_name, stat in drv_specs.items():
                                ...
                                if total_sec == 0:
                                    st.warning(f"⚠️ API通信エラー({api_err if api_err else '距離計算失敗'}): {d_name}班の計算に失敗しました。")
