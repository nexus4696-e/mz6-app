...
def render_cast_edit_card(c_id, c_name, pref, target_row, prefix_key, d_names_list, t_slots, e_t_slots, loop_idx):
    ...
    with st.expander(f"店番 {c_id} : {c_name} ({pref}) - {title_badge}"):
        ...
        new_e_drv, new_e_time, new_e_dest = ed if ed else "未定", et if et else (e_t_slots[0] if e_t_slots else "17:00"), eds
        new_stopover, new_temp_addr, new_memo, new_takuji_cancel = so, ta, m, (tc == "1")
        ...
        if show_details:
            ...
            with col_e2: new_e_time = st.selectbox("送り先到着時間", e_t_slots, index=e_t_slots.index(new_e_time) if new_e_time in e_t_slots else 0, key=f"etm_{key_suffix}")
            ...
            new_stopover = st.text_input("立ち寄り先", value=new_stopover, key=f"so_{key_suffix}")
        ...
        if st.button("💾 この内容で更新", key=f"btn_upd_{key_suffix}", type="primary", use_container_width=True):
            if n_s in ["未定", "休み"]: n_d, n_t, new_e_drv, new_e_time, new_e_dest = "未定", "未定", "未定", "未定", ""
            enc_m = encode_attendance_memo(new_memo, new_temp_addr, ("1" if new_takuji_cancel else "0"), new_e_drv, new_e_time, new_e_dest, new_stopover)
            if n_s in ["未定", "休み"]: post_api({"action": "cancel_dispatch", "cast_id": c_id})
            res = post_api({"action": "save_attendance", "records": [{"cast_id": c_id, "cast_name": c_name, "area": pref, "status": n_s, "memo": enc_m, "target_date": "当日"}]})
    ...
    with tab1:
        ...
        with st.form("f_tdy"):
            ...
            if st.form_submit_button("送信"):
                post_api({"action": "save_attendance", "records": [{"cast_id": c["店番"], "cast_name": c["キャスト名"], "area": c["方面"], "status": s, "memo": encode_attendance_memo(m, ta, "0", stopover=so_a), "target_date": "当日"}]})
    ...
    if not is_adm:
        ...
        for t in early_raw:
            ...
            if e_drv == staff_n:
                ...
                my_early.append({"task": t, "early_time": e_time, "early_dest": e_dest, "c_name": t['cast_name'], "c_id": t['cast_id'], "actual_pickup": act_pickup, "use_takuji": use_tkj, "takuji_addr": takuji_addr, "stopover": so})
