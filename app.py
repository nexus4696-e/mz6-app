...
# エラー防止：初期値を「① 配車リスト」に固定
if st.session_state.current_staff_tab is None:
...
def optimize_and_calc_route(api_key, store_addr, dest_addr, tasks_list, is_return=False):
    ...
    full_path = [clean_address_for_map(t.get("actual_pickup","")) for t in valid_tasks]
    return tasks_list, 0, full_path

# 🌟 キャスト詳細編集カード (個別LINE ＋ 管理者/担当者 権限制限)
def render_cast_edit_card(c_id, c_name, pref, target_row, prefix_key, d_names_list, t_slots, e_t_slots, loop_idx):
    ...
    with st.expander(f"店番 {c_id} : {c_name} ({pref}) - {title_badge}"):
        ...
        show_details = st.toggle("⚙️ 詳細（同伴・迎え先変更・託児等）を開く", key=f"toggle_{key_suffix}")
        if show_details:
...
<style>
    ...
    div.element-container:has(.home-title) ~ div.element-container button {
        height: 55px !important; font-size: 18px !important; margin-bottom: 12px !important;
    }
...
def render_top_nav():
    ...
    with c3: 
        if st.button("🚪 ログアウト"):
            st.session_state.logged_in_cast = st.session_state.logged_in_staff = None; st.session_state.is_admin = False; st.session_state.page = "home"; st.rerun()
...
if st.session_state.page == "home":
    ...
    c_sel = st.selectbox("店番とキャスト名", ["-- 選択 --"] + [f"{c['cast_id']} {c['name']}" for c in casts if str(c.get("name","")).strip() != ""])
