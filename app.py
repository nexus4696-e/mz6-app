...
# 🌟 Streamlitの根幹設定（フリーズの原因となる危険なJSを完全削除）
...
if page == "staff_portal":
    ...
    if is_adm:
        ...
        for i in range(1, 151):
            ...
            nm = str(c["name"])
            ad = str(c.get("address", ""))
            mgr = str(c.get("manager", "")) if c.get("manager") else "未設定"
            ...
            with col_mgr:
                ...
                if n_mgr != mgr:
                    res = post_api({"action": "save_cast", "cast_id":own("<hr style='margin: 5px 0 15px 0; border-top: 1px dashed #ccc;'>", unsafe_allow_html=True)
